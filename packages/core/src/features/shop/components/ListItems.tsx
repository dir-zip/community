      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {data.map((item) => {
          return (
            <div
              key={item.id}
              className="flex bg-primary-700 border rounded flex-col"
            >
              <img
                src={item.image as string}
                className="w-full aspect-auto object-cover rounded-tl rounded-tr h-48"
              />
              <div className="flex flex-col gap-4">
                <div className="px-4 border-t border-b bg-primary-800 py-2 flex gap-2 items-center">
                  <GoldCoinIcon />
                  <h2 className="antialiased font-bold">
                    <span className="text-xl">{item.price}</span>
                    <span className="antialiased text-xs pl-1 font-light">
                      p
                    </span>
                  </h2>
                </div>
                        ...
                        onClick={async (e) => {
                          ...
                          toast.promise(
                            ...
                            }),
                            {
                              ...
                              error: (error) =>
                                `Error buying item ${error.message}`,
                            },
                          );
                        }}
                      >
                        <div className="flex justify-center items-center">
                          ...
                          {purchaseSuccess && (
                            <Check
                              ...
                              style={{
                                opacity: purchaseSuccess[item.id] ? 1 : 0,
                              }}
                            />
                          )}
                        </div>
